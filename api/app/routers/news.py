...
def _fetch_gdelt(limit: int) -> list[dict[str, Any]]:
    query = (
        '(NYC OR "New York City" OR Manhattan OR Brooklyn OR Queens OR Bronx OR '
        '"Staten Island") (crime OR NYPD OR police OR robbery OR theft OR assault OR subway)'
    )
    ...
    for article in articles:
        ...
        has_nyc = _contains_any(title, NYC_TERMS)
        has_safety = _contains_any(title, SAFETY_TERMS)
        if not title or not has_nyc or not has_safety:
            continue
        items.append(
            {
                "title": title,
...
def _fetch_rss(limit: int) -> list[dict[str, Any]]:
    ...
    for node in root.findall("./channel/item"):
        ...
        has_nyc = _contains_any(haystack, NYC_TERMS)
        has_safety = _contains_any(haystack, SAFETY_TERMS)
        if not title or not has_nyc or not has_safety:
            continue
        published_at = None
